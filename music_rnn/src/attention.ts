...
export class AttentionWrapper {
  ...
  constructor(public cells: tf.LSTMCellFunc[],
              public attnLength: number,
  ...
  call(input: tf.Tensor2D, c: tf.Tensor2D[], h: tf.Tensor2D[],
    ...
    const attnHidden: tf.Tensor4D = tf.reshape(
      ...
      [-1, this.attnLength, 1, this.attnSize]
    );
    ...
    const attentionState: tf.Tensor2D = tf
      .concat(
        [
          newAttnStates,
