...
# Generate the docs.
npx typedoc --sourcefile-url-prefix 'https://github.com/tensorflow/magenta-js/tree/master/image/src/' --out ../docs/image src --mode modules --excludePrivate --exclude '**/*+(index|test|lib).ts' --excludeExternals

# Fix any leaked local paths in the image docs
# See https://github.com/TypeStrong/typedoc/issues/800.
cd ../docs/image/classes/
