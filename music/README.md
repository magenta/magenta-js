...
<html>
    ...
    <script>
      ...
      function generate() {
        ...
        model.sample(1).then((samples) => {
            trio = mm.sequences.sequenceProtoToMidi(samples[0]);
            player.start(trio);
        });
      }
      function dl() {
        download(new Uint8Array(mm.sequenceProtoToMidi(trio)), "trio.mid");
      }
  ...
  <body>
    ...
    <button onclick="dl()"><h1>Download Trio</h1></button>
  </body>
</html>
```

Open up that html file in your browser (or [click here](https://goo.gl/magenta/simpletrio) for a hosted version)
and the code will run. Click the generate button to hear 4-bar trios that are randomly generated by MusicVAE. If you find one you like, you can download the MIDI.
