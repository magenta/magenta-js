...
class Coconet {
  ...
  async infill(sequence: INoteSequence) {
    ...
    // Merge held notes before returning.
    return this.mergeHeldNotes(outputSequence);
  }
   ...
   * Does not merge notes that connect on a measure boundary.
   * Also rearranges the order of the notes - notes are grouped by instrument,
   * then ordered by timestamp.
   * @param sequence The original NoteSequence
   * @return a new NoteSequence with sustained notes merged.
   */
  private mergeHeldNotes(sequence: INoteSequence): NoteSequence {
