...
class Coconet {
   ...
   *
   * If the original note sequence is provided, its notes will replace
   * any notes for the first voice that the model generated.
   * @param sequence The NoteSequence to be merged
   * @param firstVoice The NoteSequence that will replace the first voice
   * generated by the model.
   * @return a new NoteSequence with sustained notes merged.
   */
  public mergeHeldNotes(sequence: INoteSequence, firstVoice?: INoteSequence):
      NoteSequence {
    ...
    output.notes = [];
    if (firstVoice) {
      for (let i = 0; i < firstVoice.notes.length; i++) {
        output.notes.push(JSON.parse(JSON.stringify(firstVoice.notes[i])));
      }
    } else {
      output.notes.push(JSON.parse(JSON.stringify(newNotes[0])));
    }

    let o = output.notes.length - 1;
    for (let i = 1; i < newNotes.length; i++) {
      const thisNote = newNotes[i];
      if (firstVoice && thisNote.instrument === 0) {
        continue;
      }
      ...
      if (previousNote.instrument === thisNote.instrument &&
        ...
        output.notes.push(JSON.parse(JSON.stringify(newNotes[i])));
        o++;
      }
    }
    return output;
