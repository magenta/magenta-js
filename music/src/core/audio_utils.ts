...
export async function resampleAndMakeMono(audioBuffer: AudioBuffer, targetSr = SAMPLE_RATE) {
	...
	if (!isSafari) {
		...
		bufferSource.start();
		return offlineCtx.startRendering().then((buffer: AudioBuffer) => buffer.getChannelData(0));
	} else {
		// Safari does not support resampling with WebAudio.
		logging.log('Safari does not support WebAudio resampling, so this may be slow.', 'O&F', logging.Level.WARN);
...
function magSpectrogram(stft: Float32Array[], power: number): [Float32Array[], number] {
...
function applyWholeFilterbank(spec: Float32Array[], filterbank: Float32Array[]): Float32Array[] {
...
function applyFilterbank(mags: Float32Array, filterbank: Float32Array[]): Float32Array {
...
function calculateMelFreqs(nMels: number, fMin: number, fMax: number): Float32Array {
