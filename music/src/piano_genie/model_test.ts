...
test('Piano Genie Model Correctness', async (t: test.Test) => {
  ...
  tf.tidy(() => {
    ...
    for (let i = 0; i < 8; ++i) {
      keys.push(genie.next(i, 1., 1337, undefined, undefined, 0.1));
    }
    // Descending pattern with argmax.
    for (let i = 7; i >= 0; --i) {
      keys.push(genie.next(i, 0., undefined, undefined, undefined, 0.1));
    }
    // Fast trill with temperature 0.5.
    for (let i = 0; i < 8; ++i) {
      keys.push(
        genie.next(3 + (i % 2), 0.5, 1337, undefined, undefined, 0.05));
    }
  ...
  tf.tidy(() => {
    ...
    // Ascending pattern with sampling.
    for (let i = 0; i < 8; ++i) {
      keys.push(genie.next(i, 1., 1337, gMajTwoOctaves, undefined, 0.1));
    }
    // Descending pattern with argmax.
    for (let i = 7; i >= 0; --i) {
      keys.push(genie.next(i, 0., undefined, gMajTwoOctaves, undefined, 0.1));
    }
    // Fast trill with temperature 0.5.
    for (let i = 0; i < 8; ++i) {
      keys.push(
        genie.next(3 + (i % 2), 0.5, 1337, gMajTwoOctaves, undefined, 0.05));
    }
  ...
  tf.tidy(() => {
    ...
    genie.next(0, undefined, undefined, undefined, -1, 0.);
    genie.next(1, undefined, undefined, undefined, 43, 0.125);
    genie.next(2, undefined, undefined, undefined, 45, 1., sampleFunc);
  });
  ...
  tf.tidy(() => {
    ...
    genie.next(1, undefined, undefined, undefined, -1, 0.125);
    genie.next(2, undefined, undefined, undefined, 44, 0.25);
    genie.next(3, undefined, undefined, undefined, 46, 1.5, sampleFunc);
  });
