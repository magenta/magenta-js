...
export default class AudioUtils {
  ...
  static melSpectrogram(
      stftEnergies: Float32Array[], melCount = 128, lowHz = 30, highHz = 8000,
      sr = SR): number[][] {
  ...
  static lazyCreateMelFilterbank(
      length: number, melCount = 20, lowHz = 30, highHz = 8000, sr = SR) {
  ...
  static createMelFilterbank(
      fftSize: number, melCount = 20, lowHz = 30, highHz = 8000, sr = SR) {
  ...
  static resampleWebAudio(audioBuffer: AudioBuffer, targetSr = SR):
      Promise<AudioBuffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      offlineCtx.oncomplete = (event) => {
        ...
        // console.log(`Resampled buffer from ${audioBuffer.length} to
        // ${len}.`);
        resolve(bufferRes);
      };
  ...
  static logSpectrogram(spec: number[][]) {
    const specT = tf.tidy(() => {
      const S = tf.tensor2d(spec);
      const magnitude = tf.abs(S);
      const refValue = 1.0;
      const amin = 1e-10;
      const topDb = 80.0;

      let logSpec = tf.log(tf.maximum(magnitude, amin))
                        .div(tf.log(10))
                        .mul(tf.scalar(10.0));
      logSpec = logSpec.sub(tf.log(tf.maximum(refValue, amin))
                                .div(tf.log(10))
                                .mul(tf.scalar(10.0)));

      logSpec = tf.maximum(logSpec, tf.max(logSpec).sub(tf.scalar(topDb)));
      return logSpec;
    });
    return tf.unstack(specT).map((t) => {
      const a = t.dataSync();
      t.dispose();
      return Array.from(a);
    });
