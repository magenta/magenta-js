...
export async function specgramsToAudio(specgrams: tf.Tensor4D) {
	const reImArray = tf.tidy(() => {
		// Synthesize audio
		const magSlice = tf.slice(specgrams, [0, 0, 0, 0], [1, -1, -1, 1]).reshape([1, 128, 1024]);
		const magMel = magSlice as tf.Tensor3D;
		const mag = melToLinear(magMel);

		const ifreqSlice = tf.slice(specgrams, [0, 0, 0, 1], [1, -1, -1, 1]).reshape([1, 128, 1024]);
...
export function istft(reIm: Float32Array[], params: InverseSpecParams): Float32Array {
